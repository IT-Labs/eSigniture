...
namespace SignNow.Net.Service
{
    public class DocumentService : AuthorizedWebClientBase, IDocumentService
    {
        ...
        public async Task<UploadDocumentResponse> UpdateDocumentFieldsAsync(string documentId, IReadOnlyCollection<ISignNowField> fields, CancellationToken cancellationToken = default)
        {
            ...
            var requestOptions = new PutHttpRequestOptions
            {
                ...
                Content = new JsonHttpContent(
                                new
                                {
                                    fields = fields.Cast<Field>().Select(x => new
                                    {
                                        x = x.JsonAttributes.X,
                                        y = x.JsonAttributes.Y,
                                        width = x.JsonAttributes.Width,
                                        height = x.JsonAttributes.Height,
                                        page_number = x.JsonAttributes.PageNumber,
                                        name = x.JsonAttributes.Name,
                                        label = x.JsonAttributes.Label,
                                        required = x.JsonAttributes.Required,
                                        prefilled_text = x.JsonAttributes.PrefilledText,
                                        role = x.RoleName,
        ...
        public async Task<UploadDocumentResponse> UpdateDocumentFieldsAsync(string documentId, IEnumerable<UpdateDocumentFields> fields, CancellationToken cancellationToken = default)
        {
            var requestedDocument = $"/document/{documentId.ValidateId()}";
            ...
            var requestOptions = new PutHttpRequestOptions
            {
                ...
                Content = new JsonHttpContent(
                                new
                                {
                                    fields = fields.Select(x => new
                                    {
                                        x = x.X,
                                        y = x.Y,
                                        width = x.Width,
                                        height = x.Height,
                                        page_number = x.PageNumber,
                                        name = x.Name,
                                        label = x.Label,
                                        required = x.Required,
                                        prefilled_text = x.PrefilledText,
                                        role = x.RoleName,
                                        type = x.Type.ToString().ToLower()
                                    })
                                }),
